...
class CylinderMagnet(ComponentBase):
    ...
    def __post_init__(self):
        ...
        # first use sympy to calculate the Bz, Bzr, Bzrr，the equation if from paper:https://doi.org/10.1016/j.jmmm.2018.02.003.(https://www.sciencedirect.com/science/article/pii/S0304885317334662), 
        # note that the def of n is different in the paper and the code, so we need to change the def of n in the code
        ...
        # use ep11, ep21, ep31, ep12, ep22, ep32 to replace the elliptic_k, elliptic_e, elliptic_pi, to avoid call elliptic_k, elliptic_e, elliptic_pi repeatedly
        Bz_subs = Bz.subs(
            {
        ...
        # use sympy.lambdify to convert the sympy expression to a numpy function
    ...
    # use numpy to calculate the Bz, Bzx, Bzxx
    ...
    def Bz_method(self, x, y, z):
        ...
        if zero_coords.size > 0:
            ...
            )#Along the Z-axis, B_z reduces to an algebraic function. Since this point is a singularity in the original expression, we need to manually replace B_z with its corresponding algebraic form.
            return BZ
    ...
    def Bzx_method(self, x, y, z):
        ...
        if zero_coords.size > 0:
            ...
            BZr[zero_coords[0][0]][zero_coords[0][1]][:] = np.zeros(len(dz)) #Along the Z-axis, B_zr would be 0 due to the symmetry
            return BZr
    ...
    def Bzxx_method(self, x, y, z):
        ...
        if zero_coords.size > 0:
            dr[zero_coords[0][0]][zero_coords[0][1]][zero_coords[0][2]] = (x[1]-x[0])/1000#I still don't have a good way to approximate this point, so I just set a small difference to 0, notice that Bzxx is symmetric.
            BZxx = (
                self.mu0_Ms
                * self.Bzrr_np(
                    r=dr,
