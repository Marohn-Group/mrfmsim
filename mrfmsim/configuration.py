...
def collection_multi_constructor(loader, tag_suffix, node):
    """Parse the "!Collection" tag into ExperimentCollection object."""

    param_dict = loader.construct_mapping(node, deep=True)

    return ExperimentCollection(name=tag_suffix, **param_dict)
...
DEFAULT_CONSTRUCTORS = {
    ...
    "multi_constructor": {
        ...
        "!Collection:": collection_multi_constructor,
    },
}
...
def experiment_multi_representer(dumper, experiment):
    ...
    expt_dict = experiment._mapped_parameters
    name = expt_dict.pop("name")

    return dumper.represent_mapping(f"!Experiment:{name}", expt_dict, flow_style=False)


def collection_multi_representer(dumper, collection):
    """Parse ExperimentCollection object into yaml string."""

    collection_dict = collection._mapped_parameters
    collection_dict["node_objects"] = NodeList(collection_dict["node_objects"])

    name = collection_dict.pop("name")
    for instructions in collection_dict["instructions"].values():
        edges = instructions["grouped_edges"]
        instructions["grouped_edges"] = BlockList(edges)

    return dumper.represent_mapping(
        f"!Collection:{name}", collection_dict, flow_style=False
    )
...
def nodes_representer(dumper, nodes):
    ...
    nodes_dict = {}
    for node in nodes:
        node_dict = node._mapped_parameters
...
DEFAULT_REPRESENTERS = [
    ...
    (ExperimentCollection, collection_multi_representer),
]
